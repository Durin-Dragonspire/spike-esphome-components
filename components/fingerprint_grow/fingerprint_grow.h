...
// ACK packet length includes its two checksum bytes. Grow modules advertise
// packet payload sizes up to 256 bytes, so anything larger cannot be a valid
// response and must not be allowed to grow the receive buffer indefinitely.
static const uint16_t MAX_ACK_PACKET_LENGTH = 258;
...
class FingerprintGrowComponent final : public PollingComponent, public uart::UARTDevice {
 public:
  ...
  // SPIKE runtime protection. Secrets are delivered transiently over the
  // encrypted native API and are never part of the generated YAML.
  // Initial provisioning/replacement probes the factory tuple first. Recovery
  // probes the last staged/final tuple first so an already-protected module
  // does not incur avoidable failed authentication attempts.
  bool spike_provision(uint32_t password, uint32_t address, bool replacement, bool recovery = false);
  bool spike_advance(uint32_t password, uint32_t address, bool replacement, bool recovery,
                     const std::string &requested_stage);
  bool spike_verify(uint32_t password, uint32_t address);
  bool spike_reset_security(uint32_t password, uint32_t address);
  bool spike_verify_pending_match(uint32_t password, uint32_t address, uint16_t finger_id);
  bool spike_refresh_inventory();
  const std::string &spike_last_result() const { return this->spike_last_result_; }
  const std::string &spike_security_state() const { return this->spike_security_state_; }
 ...
 protected:
  ...
  bool set_password_();
  bool set_module_password_(uint32_t password);
  bool set_module_address_(uint32_t address);
  bool probe_security_(uint32_t password, uint32_t address);
  bool read_index_table_();
  ...
  std::string spike_security_state_{"unverified"};
  std::string spike_occupied_slots_{};
