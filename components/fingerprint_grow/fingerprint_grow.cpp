...
bool FingerprintGrowComponent::spike_provision(uint32_t password, uint32_t address, bool replacement,
  ...
  this->spike_security_state_ = "secret_staged";
  ...
  for (size_t index = 0; index < 4U && !authenticated; index++) {
    ...
    if (authenticated || this->spike_last_result_ != "security_tuple_no_response") {
      any_response = true;
      if (!authenticated &&
          (strongest_failure.empty() || failure_rank(this->spike_last_result_) > failure_rank(strongest_failure))) {
  ...
  if (!authenticated) {
    this->security_ready_ = false;
    if (!any_response) {
      // A timeout at every known address cannot distinguish an unknown
      // address from absent UART hardware.  Report that bounded fact rather
      // than claiming the UART is dead.
      this->spike_last_result_ = "security_tuple_no_response";
  ...
  if (!this->probe_security_(password,
                             ((uint32_t) this->address_[0] << 24) |
                                 ((uint32_t) this->address_[1] << 16) |
